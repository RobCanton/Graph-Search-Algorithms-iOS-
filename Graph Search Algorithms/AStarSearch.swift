...
class AStarSearch
{
    ...
    var pq: PriorityQueue<AStarNode> = PriorityQueue<AStarNode>(ascending: true)
    ...
    func startSearch(node:cellNode, num:Int)
    {
        ...
        search(AStarNode(value: node))
    }
    
    private func search (n:AStarNode)
    {
        pq.clear()
        pq.push(n)
            ...
            if let pop:cellNode? = pq.pop()?.value
            {
                ...
                else if (pop!.isVisitable())
                {
                    ...
                    if n1 != nil && n1!.isVisitable() {
                        pq.push(AStarNode(value: n1))
                    }
                    
                    if n2 != nil && n2!.isVisitable() {
                        pq.push(AStarNode(value: n2))
                    }
                    
                    if n3 != nil && n3!.isVisitable() {
                        pq.push(AStarNode(value: n3))
                    }
                    
                    if n4 != nil && n4!.isVisitable() {
                        pq.push(AStarNode(value: n4))
                    }
